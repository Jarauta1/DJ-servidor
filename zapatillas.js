        ...
        if (err !== null) {
            ...
            if (datos.length > 0) {
                ...
                db.collection("visualizados").insertOne({titulo:titulo,cartel:cartel,id:id, visualizado: 1, edadMedia: edad, producto: producto},function(err,datos) {
                    if (err !== null) {
                        res.send({mensaje: "Error al crear visualización de zapatillas"})
                    } else {
                        res.send({creado: "si", mensaje: "Creado visualización de zapatillas"})
                    }
                })
            }   
        }
        ...
        if (err !== null) {
            ...
            if (datos.length > 0) {
                ...
                db.collection("favoritas").insertOne({titulo:titulo,cartel:cartel,id:id, edadMedia: edad, favoritas: 1, producto: producto},function(err,datos) {
                    if (err !== null) {
                        res.send({mensaje: "Error al crear favoritos de zapatillas"})
                    } else {
                        res.send({creado: "si", mensaje: "Creado el favorito de zapatillas"})
                    }
                })
            }   
        }
        ...
        if (err !== null) {
            ...
            if (datos.length > 0) {
                ...
                db.collection("cesta").insertOne({titulo:titulo,cartel:cartel, edadMedia: edad, id:id, precio:precio,cesta: 1, producto: producto},function(err,datos) {
                    if (err !== null) {
                        res.send({mensaje: "Error al crear compradas de zapatillas"})
                    } else {
                        res.send({creado: "si", mensaje: "Creado la compra de zapatillas"})
